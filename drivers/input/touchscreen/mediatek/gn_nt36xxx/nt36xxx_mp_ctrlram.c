 ...
 *
 * $Revision: 10294 $
 * $Date: 2017-03-09 16:31:40 +0800 (週四, 09 三月 2017) $
 *
...
static uint8_t RecordResult_FW_CC_I[IC_X_CFG_SIZE * IC_Y_CFG_SIZE + IC_KEY_CFG_SIZE] = {0};
...
static int32_t RawData_FW_CC_I[IC_X_CFG_SIZE * IC_Y_CFG_SIZE + IC_KEY_CFG_SIZE] = {0};
...
static int32_t nvt_read_CC(int32_t *xdata)
{
	...
	for (y = 0; y < y_num; y++) {
		for (x = 0; x < x_num; x++) {
			...
			if (ts->carrier_system) {
				RawData_FW_CC_I[iArrayIndex] = (int32_t)(xdata[iArrayIndex] & 0xFF);
				RawData_FW_CC_Q[iArrayIndex] = (int32_t)((xdata[iArrayIndex] >> 8) & 0xFF);
			} else {
			xdata[iArrayIndex] =  (int16_t)xdata[iArrayIndex];
		}
	}
	...
	for (k = 0; k < IC_KEY_CFG_SIZE; k++) {
		if (AIN_KEY[k] != 0xFF) {
			key_idx++;
			if (ts->carrier_system) {
				RawData_FW_CC_I[Y_Channel * X_Channel + AIN_KEY[k]] = (int32_t)(xdata_btn_tmp[key_idx] & 0xFF);
				RawData_FW_CC_Q[Y_Channel * X_Channel + AIN_KEY[k]] = (int32_t)((xdata_btn_tmp[key_idx] >> 8) & 0xFF);
			} else {
			xdata[Y_Channel * X_Channel + AIN_KEY[k]] = xdata_btn_tmp[key_idx];
		}
	}
...
static int32_t nvt_read_fw_noise(int32_t *xdata)
{
	...
	for (y = 0; y < y_num; y++) {
		for (x = 0; x < x_num; x++) {
			iArrayIndex = y * x_num + x;
			if (ts->carrier_system) {
				RawData_Diff_Max[iArrayIndex] = xdata[iArrayIndex];
