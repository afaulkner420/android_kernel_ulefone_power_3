 ...
 *
 * $Revision: 10756 $
 * $Date: 2017-03-22 17:30:28 +0800 (週三, 22 三月 2017) $
 *
...
// Gionee <GN_BSP_CTP> <mogongfu> <20161101>   add for TP recognition end

// Gionee <GN_BSP_CTP> <linzhanchi> <20170629>  add for ID163725 begin
//extern int touch_protect;
// Gionee <GN_BSP_CTP> <linzhanchi> <20170629>  add for ID163725 end
...
void nvt_ts_wakeup_gesture_report(uint8_t gesture_id, uint8_t point_data[64 + 2])
{
	...
	//Gionee <GN_BSP_CTP> <mogongfu> <20161026>  add for gesture coordinate begin
	...
	u16 y_max = 0;
	uint8_t i;
	//Gionee <GN_BSP_CTP> <mogongfu> <20161026>  add for gesture coordinate end
	...
	if (1 == nvt_gesture_switch) {
		switch (gesture_id) {
			...
			//Gionee <GN_BSP_CTP> <mogongfu> <20170504> modify for TP gesture begin
			...
			case GESTURE_SLIDE_RIGHT:
				...
				break;
			#endif
			//Gionee <GN_BSP_CTP> <mogongfu> <20170504> modify for TP gesture end
...
#endif /* MT_PROTOCOL_B */
				printk("gavinlin::[FINGER %d]x=%d,y=%d\n", input_id, input_x, input_y);
...
//				input_report_abs(ts->input_dev, ABS_MT_PRESSURE, input_p);
		...
		if (0 == finger_cnt)
			printk("gavinlin::all fingers up\n");
...
// Gionee <GN_BSP_CTP> <linzhanchi> <20170623>  add for ID161600 begin
/*
*	suspend mode switch
*	0 for normal mode
*	1 for gesture mode
*/
void switch_to_gesture(int mode)
{
	uint8_t buf[4] = {0};
	int gesture_try = 0;
	uint8_t gesture_buf[2] = {0};
	
	if (1 ==  mode)
	{
		for (gesture_try=0; gesture_try<5; gesture_try++)
		{
			//---write i2c command to enter "wakeup gesture mode"---
			buf[0] = EVENT_MAP_HOST_CMD;
			buf[1] = 0x13;
			gesture_buf[0] = EVENT_MAP_HOST_CMD;
			CTP_I2C_WRITE(ts->client, I2C_FW_Address, buf, 2);
			msleep(1);
			CTP_I2C_READ(ts->client,I2C_FW_Address, gesture_buf, 2);
			
			if (0x13 == gesture_buf[1])
				break;
		}

		if (5 == gesture_try)
		{
			NVT_ERR("Can not enter gesture mode\n");
			return;
		}
...
// Gionee <GN_BSP_CTP> <linzhanchi> <20170623>  add for ID161600 end
...
*/
// Gionee <GN_BSP_CTP> <linzhanchi> <20170616>  modify for ID1612517 begin
static int gn_judge_touch(void)
{
		printk("[GNCTP]touch type:%s\n",lcm_matching);
        if (0 == strncmp(lcm_matching, tm_nvt_touch, strlen(tm_nvt_touch)))
        {
                printk("[GNCTP]NT36XXX touch match\n");
                return 1;
        }
...
// Gionee <GN_BSP_CTP> <linzhanchi> <20170616>  modify for ID1612517 begin
	...
	{
		lcm_match_id = gn_judge_touch();
		if (0 == lcm_match_id)
		{
			NVT_ERR("NT36XXX IC has no match!\n");
			return -ENODEV;
		} else {
			kfree(lcm_matching);
		}
		
#ifdef CONFIG_GN_BSP_CTP_NT36XXX_HALL_SUPPORT
		//	chip_nt36672_id = 1;
#endif
	} else {
			return -ENODEV;
...
//Gionee <GN_BSP_CTP> <mogongfu> <20170408> add for TP feature begin
	gn_nvt_tpd_feature_init_data(ts->client);
//Gionee <GN_BSP_CTP> <mogongfu> <20170408> add for TP feature end
...
#ifdef CONFIG_OF
//Gionee <GN_BSP_CTP> <mogongfu> <20170323> add for tp begin
static const struct of_device_id nvt_match_table[] = {
	{.compatible = "mediatek,cap_touch"},
	{},
};
//Gionee <GN_BSP_CTP> <mogongfu> <20170323> add for tp end
#endif
...
// Gionee <GN_BSP_CTP> <linzhanchi> <20170623>  modify for ID161600 begin
if (0 == nvt_wake_switch && 0 == nvt_gesture_switch)
	enable_irq(ts->client->irq);
// Gionee <GN_BSP_CTP> <linzhanchi> <20170623>  modify for ID161600 end
