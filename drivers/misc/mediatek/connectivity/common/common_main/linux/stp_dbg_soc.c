...
#define CONSYS_REG_READ(addr) (*((volatile UINT32 *)(addr)))
	...
	if (conn_reg.mcu_base) {
		...
		case CONNSYS_DBGSR1:
			return CONSYS_REG_READ(conn_reg.mcu_base + CONSYS_DBGSR1_OFFSET);
		case CONNSYS_DBGSR2:
			return CONSYS_REG_READ(conn_reg.mcu_base + CONSYS_DBGSR2_OFFSET);
		case CONNSYS_ACR:
			return CONSYS_REG_READ(conn_reg.mcu_base + CONSYS_ACR_OFFSET);
		case CONNSYS_DBGSR:
			return CONSYS_REG_READ(conn_reg.mcu_base + CONSYS_DBGSR_OFFSET);
		case CONNSYS_DBGSTA:
			return CONSYS_REG_READ(conn_reg.mcu_base + CONSYS_DBGSTA_OFFSET);
		default:
			ret = 0;
		}
	}
	if (conn_reg.top_cr_base) {
		switch (cr) {
		case CONNSYS_RGU_BCR:
			return CONSYS_REG_READ(conn_reg.top_cr_base + CONSYS_RGU_BCR_OFFSET);
		default:
			ret = 0;
		}
	}
...
UINT32 stp_dbg_soc_write_debug_crs(ENUM_CONNSYS_DEBUG_CR cr, UINT32 data)
{
	UINT32 ret = -1;
#define CONSYS_REG_WRITE(addr, data)  mt_reg_sync_writel(data, addr)
#ifdef CONFIG_OF		/*use DT */
	if (conn_reg.mcu_base) {
		STP_DBG_INFO_FUNC("conn_reg.mcu_base = 0x%zx", conn_reg.mcu_base);
		switch (cr) {
		case CONNSYS_DBGSR2:
			CONSYS_REG_WRITE(conn_reg.mcu_base + CONSYS_DBGSR2_OFFSET, data);
			break;
		case CONNSYS_DBGSR:
			CONSYS_REG_WRITE(conn_reg.mcu_base + CONSYS_DBGSR_OFFSET, data);
			break;
		default:
			ret = 0;
		}
