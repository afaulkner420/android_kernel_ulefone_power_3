...
INT32 stp_dbg_poll_cpupcr(UINT32 times, UINT32 sleep, UINT32 cmd)
{
	...
	if (chip_type == WMT_CHIP_TYPE_SOC) {
		...
		STP_DBG_INFO_FUNC("CONNSYS DBGSR1 0x18070408:0x%08x\n",
				stp_dbg_soc_read_debug_crs(CONNSYS_DBGSR1));
		STP_DBG_INFO_FUNC("CONNSYS DBGSR2 0x1807040c:0x%08x\n",
				stp_dbg_soc_read_debug_crs(CONNSYS_DBGSR2));
		STP_DBG_INFO_FUNC("CONNSYS ACR 0x18070110:0x%08x\n",
				stp_dbg_soc_read_debug_crs(CONNSYS_ACR));
		STP_DBG_INFO_FUNC("CONNSYS DBGSR 0x18070400:0x%08x\n",
				stp_dbg_soc_read_debug_crs(CONNSYS_DBGSR));
		STP_DBG_INFO_FUNC("CONNSYS DBGSTA 0x18070404:0x%08x\n",
				stp_dbg_soc_read_debug_crs(CONNSYS_DBGSTA));
		STP_DBG_INFO_FUNC("CONNSYS RGU_BCR 0x180b0004:0x%08x\n",
				stp_dbg_soc_read_debug_crs(CONNSYS_RGU_BCR));
...
VOID stp_dbg_reset_n9(VOID)
{
	if ((g_stp_dbg_cpupcr->count >= 5) && (
			g_stp_dbg_cpupcr->buffer[g_stp_dbg_cpupcr->count-1] ==
			g_stp_dbg_cpupcr->buffer[g_stp_dbg_cpupcr->count-2]) && (
			g_stp_dbg_cpupcr->buffer[g_stp_dbg_cpupcr->count-1] ==
			g_stp_dbg_cpupcr->buffer[g_stp_dbg_cpupcr->count-3]) && (
			g_stp_dbg_cpupcr->buffer[g_stp_dbg_cpupcr->count-1] ==
			g_stp_dbg_cpupcr->buffer[g_stp_dbg_cpupcr->count-4]) && (
			g_stp_dbg_cpupcr->buffer[g_stp_dbg_cpupcr->count-1] ==
			g_stp_dbg_cpupcr->buffer[g_stp_dbg_cpupcr->count-5])) {
		stp_dbg_soc_write_debug_crs(CONNSYS_DBGSR2, 0x1);
		stp_dbg_soc_write_debug_crs(CONNSYS_DBGSR, 0x5A59);
		STP_DBG_INFO_FUNC("Reset N9 and read CONNSYS DBGSTA 0x18070404:0x%08x\n",
				stp_dbg_soc_read_debug_crs(CONNSYS_DBGSTA));
	}
}
