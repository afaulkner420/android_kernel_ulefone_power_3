...
static UINT32 consys_read_chipid(VOID);
static VOID consys_cpu_sw_rst(VOID);
static VOID consys_cpu_sw_rst_deassert(VOID);
...
WMT_CONSYS_IC_OPS consys_ic_ops = {
	...
	.consys_ic_read_chipid = consys_read_chipid,
	.consys_ic_cpu_sw_rst = consys_cpu_sw_rst,
	.consys_ic_cpu_sw_rst_deassert = consys_cpu_sw_rst_deassert,
...
static INT32 consys_read_reg_from_dts(struct platform_device *pdev)
{
	...
	if (node) {
		...
		conn_reg.top_cr_base = (SIZE_T) of_iomap(node, 4);
		WMT_PLAT_DBG_FUNC("Get top cr register base(0x%zx)\n", conn_reg.top_cr_base);
	} else {
		WMT_PLAT_ERR_FUNC("[%s] can't find CONSYS compatible node\n", __func__);
		return iRet;
	}
