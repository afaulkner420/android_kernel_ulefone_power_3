...
static int icm206xx_gyro_SetFullScale(struct i2c_client *client, u8 gyro_fsr)
{
	...
	res = icm206xx_share_read_register(ICM206XX_REG_GYRO_CFG, databuf, 1);
	if (res < 0) {
	...
	res = icm206xx_share_write_register(ICM206XX_REG_GYRO_CFG, databuf, 1);
	if (res < 0)
	{
...
static int icm206xx_gyro_SetFilter(struct i2c_client *client, u8 gyro_filter)
{
	...
	res = icm206xx_share_read_register(ICM206XX_REG_CFG, databuf, 1);
	if (res < 0) 
	{
	...
	res = icm206xx_share_write_register(ICM206XX_REG_CFG, databuf, 1);
...
/*
	...
	GYRO_LOG("read filter: 0x%x\n", databuf[0]);
*/
	return ICM206XX_SUCCESS;
}

static int icm206xx_gyro_ReadSensorDataDirect(struct i2c_client *client, s16 data[ICM206XX_AXIS_NUM])
{
	char databuf[6];
	int i;
	int res = 0;
	...
	res = icm206xx_share_read_register(ICM206XX_REG_GYRO_XH, databuf, ICM206XX_DATA_LEN);
	if (res < 0) {
...
static bool icm206xx_gyro_DoSelfTest(struct i2c_client *client)
{
	...
	// read st_code
	res = icm206xx_share_read_register(ICM206XX_REG_SELF_TEST_X_GYRO, &(st_code[0]), 3);
	if (res)
	{
		GYRO_ERR("read data fail: %d\n", res);
		return false;
...
static int icm206xx_gyro_set_delay(u64 ns)
{
	...
	if(icm206xx_share_get_sensor_power(ICM206XX_SENSOR_TYPE_GYRO) == false)	
		icm206xx_share_SetPowerMode(ICM206XX_SENSOR_TYPE_GYRO, true);
...
static int icm206xx_gyro_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/*-----------------------------------------------------------*/
	// Fill and Register the gyro_control_path and gyro_data_path
	/*-----------------------------------------------------------*/
	...
	ctl.open_report_data = icm206xx_gyro_open_report_data;	// function pointer
	ctl.enable_nodata = icm206xx_gyro_enable_nodata;		// function pointer
	ctl.set_delay  = icm206xx_gyro_set_delay;			// function pointer
	...
	data.get_data = icm206xx_gyro_get_data;			// function pointer
...
static int icm206xx_gyro_i2c_suspend(struct device *dev)
{
	...
	GYRO_LOG("icm206xx_gyro suspend ok\n");
...
static int icm206xx_gyro_i2c_resume(struct device *dev)
{
	...
	atomic_set(&obj->suspend, 0);
	GYRO_LOG("icm206xx_gyro resume ok\n");

	return 0;
}
static const struct dev_pm_ops icm206xx_g_pm_ops = {
