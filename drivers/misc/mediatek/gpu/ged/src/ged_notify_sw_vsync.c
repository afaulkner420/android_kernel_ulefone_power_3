...
static struct timer_list g_HT_hwvsync_emu;
static unsigned long g_HT_hwvsync_emu_expire_time;
 ...
 * only set the status, not really operating on real timer
 */
void timer_switch(bool bTock)
{
...
GED_ERROR ged_notify_sw_vsync(GED_VSYNC_TYPE eType, GED_DVFS_UM_QUERY_PACK* psQueryData)
{
	...
	if(GED_VSYNC_SW_EVENT==eType)
	{
		sw_vsync_ts = temp;
#ifdef ENABLE_TIMER_BACKUP
		del_timer(&g_HT_hwvsync_emu);
		g_HT_hwvsync_emu_expire_time = jiffies + msecs_to_jiffies(GED_DVFS_TIMER_TIMEOUT_MS);
		mod_timer(&g_HT_hwvsync_emu, g_HT_hwvsync_emu_expire_time);
...
static void ged_sw_vsync_check_cb(unsigned long data)
{
	unsigned long long temp;
	long long llDiff;
	GED_NOTIFY_SW_SYNC* psNotify;
...
void ged_dvfs_gpu_clock_switch_notify(bool bSwitch)
{
	if (bSwitch) {
		...
		if (g_timer_on)
			...
			if (del_timer(&g_HT_hwvsync_emu))
				ged_log_buf_print(ghLogBuf_DVFS, "[GED_K] Timer should be active");
			g_HT_hwvsync_emu_expire_time = jiffies + msecs_to_jiffies(GED_DVFS_TIMER_TIMEOUT_MS);
			mod_timer(&g_HT_hwvsync_emu, g_HT_hwvsync_emu_expire_time);
...
GED_ERROR ged_notify_sw_vsync_system_init(void)
{
	...
	init_timer_deferrable(&g_HT_hwvsync_emu);
	g_HT_hwvsync_emu.function = ged_sw_vsync_check_cb;
	g_HT_hwvsync_emu.data = (unsigned long)&g_HT_hwvsync_emu;
	g_HT_hwvsync_emu_expire_time = 0;
