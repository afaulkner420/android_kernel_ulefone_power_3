...
static void
kbasep_pm_do_gpu_poweroff_callback(unsigned long data)
{
	...
	if (kbdev->pm.backend.poweroff_timer_needed) {
		...
		timer->expires = jiffies + kbdev->pm.mtk_gpu_poweroff_time;
...
static void kbasep_pm_do_gpu_poweroff_wq(struct work_struct *data)
{
	...
	if (do_poweroff) {
		...
		del_timer(&kbdev->pm.backend.gpu_poweroff_timer);
		kbdev->pm.backend.poweroff_timer_running = false;
...
int kbase_pm_policy_init(struct kbase_device *kbdev)
{
	...
	init_timer_deferrable(&kbdev->pm.backend.gpu_poweroff_timer);
	kbdev->pm.backend.gpu_poweroff_timer.function =
			kbasep_pm_do_gpu_poweroff_callback;
	kbdev->pm.backend.gpu_poweroff_timer.data =
			(unsigned long)&kbdev->pm.backend.gpu_poweroff_timer;
	...
	kbdev->pm.mtk_gpu_poweroff_time = ((HZ * 4) / 10000) ? ((HZ * 4) / 10000) : 1;
...
void kbase_pm_update_active(struct kbase_device *kbdev)
{
	...
	if (active) {
		...
		if (!backend->poweroff_timer_running && !backend->gpu_powered &&
			...
			backend->gpu_poweroff_timer.expires = jiffies + kbdev->pm.mtk_gpu_poweroff_time;
			mod_timer(&backend->gpu_poweroff_timer, backend->gpu_poweroff_timer.expires);
		}
