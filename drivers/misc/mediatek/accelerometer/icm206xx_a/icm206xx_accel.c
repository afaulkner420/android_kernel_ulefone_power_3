...
/*
	if ( (icm206xx_accel_debug.ucPrevCmd == converted_cmd) && (converted_cmd == 2) )
	{
		icm206xx_accel_debug.nDriverState = 1;
	}
	else
*/	{
		icm206xx_accel_debug.nDriverState = 0;

		if ( (icm206xx_accel_debug.nLogIndex < DEBUG_MAX_IOCTL_CMD_COUNT) && 
...
static void printCmdLog(void)
{
	...
	for (i = 0; i < nRowCount + 1; i++)
	{
		sprintf(&icm206xx_accel_debug.strDebugDump[i * 24], "%02x,%02x,%02x,%02x,%02x,%02x,%02x,%02x\n",
				...
				icm206xx_accel_debug.ucCmdLog[i*8+5],
				icm206xx_accel_debug.ucCmdLog[i*8+6],
				icm206xx_accel_debug.ucCmdLog[i*8+7]);
	}
...
static int icm206xx_accel_SetFullScale(struct i2c_client *client, u8 accel_fsr)
{
	...
	res = icm206xx_share_read_register(ICM206XX_REG_ACCEL_CFG, databuf, 1);
	if (res < 0) {
	...
	res = icm206xx_share_write_register(ICM206XX_REG_ACCEL_CFG, databuf, 1);
	if (res < 0)
	{
...
static int icm206xx_accel_SetFilter(struct i2c_client *client, u8 accel_filter)
{
	...
	res = icm206xx_share_read_register(ICM206XX_REG_ACCEL_CFG2, databuf, 1);
	if (res < 0) {
	...
	res = icm206xx_share_write_register(ICM206XX_REG_ACCEL_CFG2, databuf, 1);
...
static int icm206xx_accel_ReadSensorDataDirect(struct i2c_client *client, s16 data[ICM206XX_AXIS_NUM])
{
	...
	if (client == NULL) return ICM206XX_ERR_INVALID_PARAM;

	res = icm206xx_share_read_register(ICM206XX_REG_ACCEL_XH, databuf, ICM206XX_DATA_LEN);
	if (res < 0) {
...
static int icm206xx_accel_ReadOffsetData(struct i2c_client *client, char *buf)
{
	...
	/* offset value [0, 0, 0] */
	sprintf(buf, "%04x %04x %04x", 	obj->offset[ICM206XX_AXIS_X], 
					obj->offset[ICM206XX_AXIS_Y], 
					obj->offset[ICM206XX_AXIS_Z]);
...
static bool icm206xx_accel_DoSelfTest(struct i2c_client *client)
{
	...
	// read st_code
	res = icm206xx_share_read_register(ICM206XX_REG_SELF_TEST_X_ACC, &(st_code[0]), 3);
	if (res)
	{
		ACC_ERR("Read data fail: %d\n", res);
		return false;
...
static void icm206xx_accel_eint_work(struct work_struct * work)
{
	extern void icm206xx_sc_NotifySensorData(int int_type);
	...
	if (obj != NULL)
	{
		res = icm206xx_share_read_register(ICM206XX_REG_DMP_INT_STATUS, databuf, 1);

		icm206xx_sc_NotifySensorData(databuf[0]);
		ACC_LOG("eint work calls NotifySensorData with [int_type:%d]\n", databuf[0]);

		enable_irq(obj->irq);
	}
}

static irqreturn_t icm206xx_accel_eint_handler(int irq, void *arg)
{
	struct icm206xx_accel_i2c_data *obj = obj_i2c_data;
...
static int icm206xx_accel_setup_eint(void)
{
	...
	// PltFmDev = get_accel_platformdev();
	// if(PltFmDev == NULL)
	// {
	// 	ACC_ERR("Cannot find accel platform!\n");
	// }

	// /* gpio setting */
	// pinctrl = devm_pinctrl_get(&PltFmDev->dev);
	// if (IS_ERR(pinctrl)) {
	// 	res = PTR_ERR(pinctrl);
	// 	ACC_ERR("Cannot find accel pinctrl!\n");
	// }

	// pins_default = pinctrl_lookup_state(pinctrl, "pin_default");
	// if (IS_ERR(pins_default)) {
	// 	res = PTR_ERR(pins_default);
	// 	ACC_ERR("Cannot find accel pinctrl default!\n");
	// }

	// pins_cfg = pinctrl_lookup_state(pinctrl, "pin_cfg");
	// if (IS_ERR(pins_cfg)) {
	// 	res = PTR_ERR(pins_cfg);
	// 	ACC_ERR("Cannot find accel pinctrl pin_cfg!\n");
	// }

	/* eint request */
	// Taly : of_find_compatible_node function gets wrong irq_node (gsensor i2c device), so, use dev.of_node information
	irq_node = of_find_compatible_node(NULL, NULL, "mediatek, gyro-eint");	
	// irq_node = PltFmDev->dev.of_node;
...
static ssize_t show_register_dump(struct device_driver *ddri, char *buf)
{
	...
	for(i = 0; i < 0x7F; i++)
	{
		...
		{
			res = icm206xx_share_read_register(i, databuf, 1);
			if (res < 0) {
...
static int icm206xx_accel_set_delay(u64 ns)
{
	...
	/* Power Mode setting in case of set_delay is called before enable_nodata */
	if(icm206xx_share_get_sensor_power(ICM206XX_SENSOR_TYPE_ACC) == false)	
		icm206xx_share_SetPowerMode(ICM206XX_SENSOR_TYPE_ACC, true);
...
#ifdef ICM206XX_USE_INTERRUPT
	// Interrupt
	INIT_WORK(&obj->eint_work, icm206xx_accel_eint_work);
#endif
...
static int icm206xx_accel_i2c_resume(struct device *dev)
{
	...
	atomic_set(&obj->suspend, 0);
	ACC_LOG("icm206xx_accel resume ok\n");

	return 0;
}
static const struct dev_pm_ops icm206xx_a_pm_ops = {
