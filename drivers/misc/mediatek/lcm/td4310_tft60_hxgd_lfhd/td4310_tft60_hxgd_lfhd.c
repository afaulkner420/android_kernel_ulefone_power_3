...
#if defined(BUILD_LK)
    printf("%s,td4310_tft60_hxgd_lfhd buffer[0] = 0x%08x, id = 0x%08x\n", __func__, buffer[0], id);
#else
    printk("%s,td4310_tft60_hxgd_lfhd buffer[0] = 0x%08x, buffer[1] = 0x%08x, id = 0x%08x\n", __func__, buffer[0], buffer[1], id);
#endif
    return (0x023c == id)?1:0;
}

LCM_DRIVER td4310_tft60_hxgd_lfhd_lcm_drv =
{
    .name			= "td4310_tft60_hxgd_lfhd",
