...
#define S5K3P3SX_OTP_DEVICE_ID							0xA2
...
//#define CAM_CAL_DRVNAME "S5K3P3SX_OTP_CAM_CAL_DRV"
...
bool s5k3p3sx_otp_selective_read_byte(u32 addr, u8 *data, u16 i2c_id)
{
	/* CAM_CALDB("s5k3p3sx_otp_selective_read_byte\n"); */
	...
	if(iReadRegI2C((u8 *)(&offset), 2, (u8*)data, 1, i2c_id)<0){
		CAM_CALERR("fail s5k3p3sx_otp_selective_read_byte addr =0x%x data = 0x%x, offset 0x%x", addr, *data,offset);
		return false;
	}
	return true;
}
...
unsigned int s5k3p3sx_otp_selective_read_region(u32 addr, u8 *data, u16 i2c_id, u32 size)
{
	...
	CAM_CALDB("s5k3p3sx_otp_selective_read_region start addr =%x size %d\n", addr, size);
	while (size_to_read > 0) {
		if (s5k3p3sx_otp_selective_read_byte(addr, (u8*)buff, i2c_id)) {
			addr += 1;
			buff += 1;
			size_to_read -= 1;
			ret += 1;
	...
	CAM_CALDB("s5k3p3sx_otp_selective_read_region end addr =%x size %d data read = %d\n", addr, size, ret);
	return ret;
}
...
static long s5k3p3sx_otp_Ioctl_Compat(struct file *filp, unsigned int cmd, unsigned long arg)
{
	long ret = 0, err = 0;
	struct COMPAT_stCAM_CAL_INFO_STRUCT *data32 = NULL;
	struct stCAM_CAL_INFO_STRUCT __user *data = NULL;

	printk("[CAMERA SENSOR] s5k3p3sx_otp_Ioctl_Compat,%p %p %x ioc size %d\n",
...
#endif
{
	...
	if (_IOC_DIR(a_u4Command) != _IOC_NONE) {
		...
		if (_IOC_WRITE & _IOC_DIR(a_u4Command)) {
			if (copy_from_user((u8 *) pBuff, (u8 *) a_u4Param, sizeof(struct stCAM_CAL_INFO_STRUCT))) {
				...
				kfree(pBuff);
				CAM_CALDB("[S5K3P3SX_OTP_CAL] ioctl copy from user failed\n");
				return -EFAULT;
			}
		}
	}
	...
	case CAM_CALIOC_S_WRITE:
		printk("[S5K3P3SX_OTP_CAL] Write CMD\n");
#ifdef CAM_CALGETDLT_DEBUG
	...
	case CAM_CALIOC_G_READ:
		printk("[S5K3P3SX_OTP_CAL] Read CMD\n");
#ifdef CAM_CALGETDLT_DEBUG
		do_gettimeofday(&ktv1);
#endif
		...
		printk("[CAM_CAL] length %d\n", ptempbuf->u4Length);
            ...
            i4RetValue = s5k3p3sx_otp_selective_read_region(ptempbuf->u4Offset, pu1Params, S5K3P3SX_OTP_DEVICE_ID,ptempbuf->u4Length);
		...
		printk("[S5K3P3SX_OTP_CAL] After read Working buffer data  0x%x\n", *pu1Params);
		...
		break;
	default:
		CAM_CALDB("[S5K3P3SX_OTP_CAL] No CMD\n");
		i4RetValue = -EPERM;
		break;
	}
	...
	if (_IOC_READ & _IOC_DIR(a_u4Command)) {
		...
		CAM_CALDB("[S5K3P3SX_OTP_CAL] to user length %d\n", ptempbuf->u4Length);
		CAM_CALDB("[S5K3P3SX_OTP_CAL] to user  Working buffer address 0x%p\n", pu1Params);
		if (copy_to_user((u8 __user *) ptempbuf->pu1Params, (u8 *)pu1Params, ptempbuf->u4Length)) {
			kfree(pBuff);
			kfree(pu1Params);
			CAM_CALDB("[S5K3P3SX_OTP_CAL] ioctl copy to user failed\n");
			return -EFAULT;
		}
	}
...
static int CAM_CAL_Open(struct inode *a_pstInode, struct file *a_pstFile)
{
	...
	printk("[S5K3P3SX_OTP_CAL] CAM_CAL_Open\n");
	spin_lock(&g_CAM_CALLock);
	if (g_u4Opened) {
		//spin_unlock(&g_CAM_CALLock);
		CAM_CALDB("[S5K3P3SX_OTP_CAL] Opened, return -EBUSY\n");
		ret = -EBUSY;
	} else {
		g_u4Opened = 1;
...
{
	printk("[S5K3P3SX_OTP_CAL] CAM_CAL_Release\n");
	spin_lock(&g_CAM_CALLock);
...
static const struct file_operations g_stCAM_CAL_fops = {
	...
	/* .ioctl = CAM_CAL_Ioctl */
#ifdef CONFIG_COMPAT
	.compat_ioctl = s5k3p3sx_otp_Ioctl_Compat,
#endif
...
static inline int RegisterCAM_CALCharDrv(void)
{
	...
	if (alloc_chrdev_region(&g_CAM_CALdevno, 0, 1, CAM_CAL_DRVNAME)) {
		CAM_CALDB("[S5K3P3SX_OTP_CAL] Allocate device no failed\n");

		return -EAGAIN;
	}
#else
	if (register_chrdev_region(g_CAM_CALdevno, 1, CAM_CAL_DRVNAME)) {
		CAM_CALDB("[S5K3P3SX_OTP_CAL] Register device no failed\n");
	...
	if (g_pCAM_CAL_CharDrv == NULL) {
		...
		CAM_CALDB("[S5K3P3SX_OTP_CAL] Allocate mem for kobject failed\n");
	...
	if (cdev_add(g_pCAM_CAL_CharDrv, g_CAM_CALdevno, 1)) {
		CAM_CALDB("[S5K3P3SX_OTP_CAL] Attatch file operation failed\n");
...
static const struct i2c_device_id CAM_CAL_i2c_id[] = {{CAM_CAL_DRVNAME, 0}, {} };
    ...
    { .compatible = "mediatek,cam_cal_drv_s5k3p3sx_otp", },
    {}
    };
...
static int CAM_CAL_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	CAM_CALDB("[S5K3P3SX_OTP_CAL] Attach I2C\n");
	/* spin_lock_init(&g_CAM_CALLock); */

	/* get sensor i2c client */
	spin_lock(&g_CAM_CALLock); /* for SMP */
	g_pstI2Cclient = client;
	g_pstI2Cclient->addr = S5K3P3SX_OTP_DEVICE_ID >> 1;
	...
	if (i4RetValue) {
		CAM_CALDB("[S5K3P3SX_OTP_CAL] register char device failed!\n");
		return i4RetValue;
	}


	CAM_CALDB("[S5K3P3SX_OTP_CAL] Attached!!\n");
	return 0;
}
...
static int __init CAM_CAL_i2C_init(void) 
{
	...
	if (platform_driver_register(&g_stCAM_CAL_Driver)) {
		CAM_CALDB("failed to register S5K3P3SX_OTP_CAL driver\n");
		return -ENODEV;
	}

	if (platform_device_register(&g_stCAM_CAL_Device)) {
		CAM_CALDB("failed to register S5K3P3SX_OTP_CAL driver, 2nd time\n");
		return -ENODEV;
	}
