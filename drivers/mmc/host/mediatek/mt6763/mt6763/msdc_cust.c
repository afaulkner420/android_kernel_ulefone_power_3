...
static void msdc_dump_clock_sts_core(struct msdc_host *host, struct seq_file *m)
{
	...
	if (topckgen_base && pericfg_base) {
		buf_ptr += sprintf(buf_ptr,
			"MSDC0 HCLK_MUX@0x%p = 0x%x,%d\n",
			topckgen_base + 0x070,
			/* mux at bits8~9 */
			(MSDC_READ32(topckgen_base + 0x070) >> 8) & 3,
			/* pdn at bit15*/
			(MSDC_READ32(topckgen_base + 0x070) >> 15) & 1);
		buf_ptr += sprintf(buf_ptr,
			"MSDC0 CLK_MUX@0x%p = 0x%x,%d, CLK_CG[0x%p] = %d,%d, [0x%p] = %d,%d\n",
			topckgen_base + 0x070,
			/* mux at bits16~18 */
			...
			infracfg_ao_base + 0x0c8,
			/* cg at bit 7, 8 */
			(MSDC_READ32(infracfg_ao_base + 0x0c8) >> 7) & 1,
			(MSDC_READ32(infracfg_ao_base + 0x0c8) >> 8) & 1);
		buf_ptr += sprintf(buf_ptr,
			"MSDC1 CLK_MUX@0x%p = 0x%x,%d CLK_CG[0x%p] = %d,%d\n",
			topckgen_base + 0x070,
			/* mux at bits24~26 */
		...
		buf_ptr += sprintf(buf_ptr,
			"MSDC2 CLK_MUX@0x%p = 0x%x,%d CLK_CG[0x%p] = %d,%d\n",
			topckgen_base + 0x080,
			/* mux at bits0~2 */
			...
			infracfg_ao_base + 0x094,
			/* cg at bit 5, 17 */
			(MSDC_READ32(infracfg_ao_base + 0x094) >> 5) & 1,
			(MSDC_READ32(infracfg_ao_base + 0x094) >> 17) & 1);
...
void msdc_dump_padctl_by_id(u32 id)
{
	...
	if (id == 0) {
		...
		pr_err("MSDC1 TDSEL0 [0x%p] =0x%8x\n",
		...
		pr_err("MSDC1 DRV0   [0x%p] =0x%8x\n",
			MSDC1_GPIO_DRV0_ADDR,
			MSDC_READ32(MSDC1_GPIO_DRV0_ADDR));
		pr_err("MSDC1 PUPD0  [0x%p] =0x%8x\tshould: 0x??222226\n",
...
void msdc_set_pin_mode(struct msdc_host *host)
{
	...
	if (host->id == 0) {
		...
		if (chk_pin_sta == 1) {
			...
			pr_err("msdc0: error! pin state not init\n");
		}
	} else if (host->id == 1) {
