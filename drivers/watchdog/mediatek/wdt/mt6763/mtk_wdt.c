					...
					bool wdt_en)
{
	...
	if (!wdt_suspended) {
		pr_debug("[wdt] mode change to 0x%x (write 0x%x), pid: %d\n",
...
static int mtk_wdt_probe(struct platform_device *dev)
{
	...
	pr_info("[wdt] mtk wdt driver probe ..\n");

	if (!toprgu_base) {
		toprgu_base = of_iomap(dev->dev.of_node, 0);
		if (!toprgu_base) {
			pr_info("[wdt] iomap failed\n");
			return -ENODEV;
		}
	}
	...
	if (!wdt_irq_id) {
		...
		if (!wdt_irq_id) {
			pr_info("[wdt] get wdt_irq_id failed, ret: %d\n", wdt_irq_id);
			return -ENODEV;
		}
	}
	...
	if (!wdt_sspm_irq_id) {
		...
		if (!wdt_sspm_irq_id) {
			 ...
			 */
			pr_info("[wdt] get wdt_sspm_irq_id failed, ret: %d\n", wdt_sspm_irq_id);
			wdt_sspm_irq_id = 0;
		}
	}

	pr_debug("[wdt] base: 0x%p, wdt_irq_id: %d, wdt_sspm_irq_id: %d\n",
	...
	pr_info("[wdt]: ext_debugkey_io: %d\n", ext_debugkey_io);

#ifndef __USING_DUMMY_WDT_DRV__ /* FPGA will set this flag */

#ifndef CONFIG_FIQ_GLUE
	pr_debug("[wdt] !CONFIG_FIQ_GLUE: request IRQ\n");
    #ifdef CONFIG_KICK_SPM_WDT
...
#else
	pr_debug("[wdt] CONFIG_FIQ_GLUE: request FIQ\n");
    #ifdef CONFIG_KICK_SPM_WDT
	...
	if (ret != 0) {
		pr_info("[wdt] failed to request wdt_irq_id %d, ret %d\n", wdt_irq_id, ret);
		return ret;
	}
	...
	if (wdt_sspm_irq_id) {
		...
		if (ret != 0) {
			pr_info("[wdt] failed to request wdt_sspm_irq_id %d, ret %d\n", wdt_sspm_irq_id, ret);
	...
	pr_debug("[wdt] WDT (dual mode) enabled.\n");
	mtk_wdt_mode_config(TRUE, TRUE, TRUE, FALSE, TRUE);
	#else				/* Initialize to disable wdt */
	pr_debug("[wdt] WDT disabled.\n");
	...
	udelay(100);
	pr_debug("[wdt] WDT_MODE(0x%x), WDT_NONRST_REG(0x%x)\n",
		__raw_readl(MTK_WDT_MODE), __raw_readl(MTK_WDT_NONRST_REG));
	pr_debug("[wdt] WDT_REQ_MODE(0x%x)\n", __raw_readl(MTK_WDT_REQ_MODE));
	pr_debug("[wdt] WDT_REQ_IRQ_EN(0x%x)\n", __raw_readl(MTK_WDT_REQ_IRQ_EN));
